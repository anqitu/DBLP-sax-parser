...
publication_df_keep, publication_df_drop = train_test_split(publication_df, test_size=0.375, random_state=0, stratify = publication_df['pubType'])
publication_df['pubType'].value_counts().sum() / 2
...
for df_path, crossref in {pub_subclass_csv_path['article']: 'articleCrossref',\
    ...
    pub_df.to_csv(df_path.replace('/csv/', '/csv_half/'), index = False, sep = '|', line_terminator='\r\n')
    print(df_path.replace('/csv/', '/csv_half/'))
    print(pub_df.shape)
...
for pub_path in pub_subclass_csv_path.values():
    pub_df = pd.read_csv(pub_path.replace('/csv/', '/csv_half/'), sep = '|')
...
authorship_df.to_csv(authorship_csv_path.replace('/csv/', '/csv_half/'), index = False, sep = '|', line_terminator='\r\n')
editorship_df = editorship_df[editorship_df['pubKey'].isin(publication_df_keep['pubKey'])]
editorship_df.to_csv(editorship_csv_path.replace('/csv/', '/csv_half/'), index = False, sep = '|', line_terminator='\r\n')
...
alias_df_keep.shape
alias_df_keep.to_csv(alias_csv_path.replace('/csv/', '/csv_half/'), index = False, sep = '|', line_terminator='\r\n')
...
person_df_keep.to_csv(person_csv_path.replace('/csv/', '/csv_half/'), index = False, sep = '|', line_terminator='\r\n')
